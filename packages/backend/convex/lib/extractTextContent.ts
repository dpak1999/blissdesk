...
const AI_MODELS = {
  image: google.chat("gemini-2.0-flash-lite"),
...
const SYSTEM_PROMPT = {
  image:
    "You turn images into text. If it is a photo of a document, transcribe it. If it is not a document then describe it.",
...
export async function extractTextContent(
  ...
  const { bytes, filename, mimeType, storageId } = args;
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "Extract the text from PDF and print it without explaining you will do so.",
          },
        ],
      },
    ],
